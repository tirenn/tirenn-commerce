...
- `[Frontend]` Unified 100% of Web API Requests to Golang Backend (`:8080`):
  - Updated `src/services/api.ts`, `AdminAIChatDrawer.tsx`, and `KnowledgeManagement.tsx` to route all AI endpoints (`/chat/shopper`, `/chat/admin`, `/knowledge/*`, `/catalog/search`) through the compiled Golang backend.
