...
func (r *repository) GetRecommendations(ctx context.Context, productID uint, limit int) ([]Product, error) {
	if limit <= 0 {
		limit = 6
	}

	var target Product
	if err := r.db.WithContext(ctx).Select("id, category_id, sub_category_id, embedding").Where("id = ?", productID).First(&target).Error; err != nil {
		return nil, err
	}
	...
	if target.Embedding != nil && len(target.Embedding.Slice()) > 0 {
		...
		err := r.db.WithContext(ctx).
			...
			Raw(`
				...
				ORDER BY 
					CASE 
						WHEN p.sub_category_id IS NOT NULL AND p.sub_category_id = ? THEN (p.embedding <=> ?) - 0.15
						WHEN p.category_id = ? THEN (p.embedding <=> ?) - 0.08
						ELSE (p.embedding <=> ?)
					END ASC
				LIMIT ?
	...
	// Fallback to category top sellers
	return r.GetCategoryTopSellers(ctx, target.CategoryID, productID, limit)
}
