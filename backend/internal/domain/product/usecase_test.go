...
func (m *mockProductRepo) GetRecommendations(ctx context.Context, productID uint, limit int) ([]Product, error) {
	if _, ok := m.products[productID]; !ok {
		return nil, errors.New("product not found")
	}
	var list []Product
	for id := uint(1); id <= uint(len(m.products)+100); id++ {
		if p, ok := m.products[id]; ok {
			if p.ID != productID && p.IsActive {
				list = append(list, *p)
				if len(list) >= limit {
					break
				}
...
func TestProductUseCase_GetRecommendations_Success(t *testing.T) {
	repo := newMockProductRepo()
...
func TestProductUseCase_GetRecommendations_FallbackToCategory(t *testing.T) {
	repo := newMockProductRepo()
	repo.products[1] = &Product{ID: 1, CategoryID: 1, Name: "Base Product", IsActive: true}
...
func TestProductUseCase_GetRecommendations_ProductNotFound(t *testing.T) {
	...
	if !errors.Is(err, domain.ErrNotFound) {
		t.Errorf("Expected domain.ErrNotFound error, got %v", err)
	}
}
